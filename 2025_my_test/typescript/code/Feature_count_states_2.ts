...
abstract class Code_Writer {
    ...
    protected write(a:string) { this.arr.push(a); }
    protected writeln(a:string) { this.arr.push(a + "<br>"); }
}

class Simple_Code_Writer extends Code_Writer {
    constructor(arr: any[]) { super(arr); }
    ...
    write_code_block(hasError: boolean, lines) {
        let error_pos = -1;
    ...
    getRandomActionNameOld(count: number): string[] {
        const chars = ['k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u'];
        if (count > chars.length) throw new Error('Too many requested characters');
        return this.fisherYatesShuffle(chars).slice(0, count);
    }

    getRandomActionName(count: number, states: string[]): string[] {
        ...
        if (count > availableChars.length) throw new Error('Too many requested characters');

        return this.fisherYatesShuffle(availableChars).slice(0, count);
    }

    getRandomStateName(count: number): string[] {
        const chars = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u'];
        if (count > chars.length) throw new Error('Too many requested characters');
        return this.fisherYatesShuffle(chars).slice(0, count);
    }
    ...
    removeRandomElements<T>(array: T[], count: number): T[] {
        const copy = [...array];
    ...
    calculate_expression(expr: string): number {
    ...
    get_actions_from_types(actionNames: string[], stateNames: string[], actionTypes: string[][]) {
        ...
        actionNames.forEach(actionName => {
            ...
            const processedLines = action_type.map(line => {
                ...
                newLine = newLine.replace(/\$state/g, () => {
                    stateCount++;
                    if (stateCount === 1) return stateValue1;
                    if (stateCount === 2) return stateValue2;
    ...
    get_actions_from_types_with_difficulty(actionNames: string[], stateNames: string[], actionTypes: string[][], difficulty: number, targetState: string, smt: string) {
        ...
        const targetStateAppears: boolean = Math.random() < 0.5;
            ...
            const processedLines = action_type.map(line => {
                ...
                if (smt.includes("Context")) {
                    ...
                    newLine = newLine.replace(/\$state/g, () => {
                        ...
                        if (stateCount === 1 && difficulty == 1 && isLast && targetStateAppears) return targetState.toUpperCase();
                        if (stateCount === 1) return stateValue1.toUpperCase();
                        if (stateCount === 2) return stateValue2;
                        return stateValue3;  // Return stateValue3 for the third $state
                    });
                } else {
                    newLine = newLine.replace(/\$state/g, () => {
                        stateCount++;
                        // the action_type on isLast is already the easy one (just one State)
                        if (stateCount === 1 && difficulty == 1 && isLast && targetStateAppears) return targetState;
                        if (stateCount === 1) return stateValue1;
                        if (stateCount === 2) return stateValue2;
                        return stateValue3;  // Return stateValue3 for the third $state
                    });
    ...
    get_actions_from_types_with_error(actionNames: string[], stateNames: string[], actionTypes: string[][], hasError: boolean) {
        const actions: string[][] = [];
...
// JOTAI PART
    ...
    get_jotai_states(stateNames: string[]) {
        const states = [];

        const initialStateValues: Record<string, number> = {}; // We will store initial values here
    ...
    write_jotai_code(hasError: boolean, difficulty: number ,smt: string, errorType: number, correctStateGroup: number) {
...
// REDUX PART
    ...
    write_redux_code(hasError: boolean, difficulty: number, smt: string, errorType: number, correctStateGroup: number) {
        // Step 1: Determine number of correct states
        ...
        const actions = this.get_actions_from_types_with_difficulty(
            actionNames,
            fullPool,
            this.get_redux_action_types(),
            difficulty,
            targetState,
            smt
        );
        ...
        let adjustedStateNames1: string[] = [];
        let adjustedStateNames2: string[] = [];

        if (!hasError) {
            adjustedStateNames1 = [...correctStateNames];
            adjustedStateNames2 = [...correctStateNames];
        } else {
            const falseStates1: string[] = [];
            const falseStates2: string[] = [];
            ...
            for (const name of shuffledRemaining) {
                ...
                if (target === 1) falseStates1.push(name);
                else falseStates2.push(name);
            }
        ...
        const lines = [
            ...
            ...shuffledInitial.map((stateName) => {
                const value = initialStateMap[stateName] ?? this.getRandomInt(10);
    ...
    write_redux_code_old(hasError: boolean, difficulty: number, smt: string, errorType: number) {
        const amountStates = this.getRandomIntBetween(3,9);
        const amountActions = this.getRandomIntBetween(3,6);
        const stateNames = this.getRandomStateName(amountStates);
        const actionNames = this.getRandomActionName(amountActions, stateNames);
...
// ZUSTAND PART
    ...
    write_zustand_code(hasError: boolean, difficulty: number, smt: string, errorType: number, correctStateGroup: number) {
        // Step 1: Determine number of correct states
        let amountCorrectStates = 0;
        if (correctStateGroup === 0) {
            amountCorrectStates = this.getRandomIntBetween(0, 3);
        ...
        const actions = this.get_actions_from_types_with_difficulty(
            actionNames,
            totalAvailableStateNames,
            this.get_zustand_action_types(),
            difficulty,
            targetState,
            smt
        );
        ...
        const statesAndActionsBeforeShuffle: string[] = [
            ...adjustedStateNames1.map(stateName => `   ${stateName}: number,`),
        ...
        for (const name of adjustedStateNames2) {
            entries.push(`   ${name}: ${initialStateMap[name] ?? this.getRandomInt(10)},`);
        }
    ...
    write_zustand_code_old(hasError: boolean, difficulty:number, smt: string, errorType: number) {
        const amountStates = this.getRandomIntBetween(3,9);
        const amountActions = this.getRandomIntBetween(3,6);
        const stateNames = this.getRandomStateName(amountStates);
        const actionNames = this.getRandomActionName(amountActions, stateNames);
...
// CONTEXT PART
    ...
    write_context_code(hasError: boolean, difficulty: number, smt: string, errorType: number, correctStateGroup: number) {
        ...
        let adjustedStateNames1: string[] = [];
        let adjustedStateNames2: string[] = [];
        let adjustedStateNames3: string[] = [];
        ...
        if (!hasError) {
            ...
            for (const name of shuffledRemaining) {
                ...
                if (targetLists.includes(1)) extra1.push(name);
                if (targetLists.includes(2)) extra2.push(name);
                if (targetLists.includes(3)) extra3.push(name);
            }
        ...
        for (const name of adjustedStateNames2) {
            entries.push(`   const [${name}, set${name.toUpperCase()}] = useState<number>(${initialStateMap[name] ?? this.getRandomInt(10)});`);
        }
    ...
    write_context_code_old(hasError: boolean, difficulty: number, smt: string, errorType: number, correctStateAmount: number) {
        let amountCorrectStates = 0;
...
export class Code_Generation_Task {
    ...
    answer: string;
    difficulty: number;
    errorType: number; //1 is Error in API //2 is Error in Protokoll
    correctStateGroup: number; // 1-3 where 1=0,1,2; 2=3,4,5; 3=6,7,8
...
export function get_tasks_explanation(smt: string) {
